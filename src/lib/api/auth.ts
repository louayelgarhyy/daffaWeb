...
export async function login(data: LoginRequest): Promise<LoginResponse> {
  const response = await post<LoginResponse>("/api/v2/user/login", data, false);
...
export async function registerCode(data: RegisterCodeRequest): Promise<RegisterCodeResponse> {
  const response = await post<RegisterCodeResponse>("/api/v2/user/register_code", data, false);
